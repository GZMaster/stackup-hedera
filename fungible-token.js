...
async function createFungibleToken() {
	...
	console.log(
		`- Token association with Bob's account: ${associateBobRx.status}\n`
	);

	// Part 3 - Check balance (before)
	var aliceBefore = await new AccountBalanceQuery()
		.setAccountId(aliceId)
		.execute(client);
	console.log(
		`- Alice balance: ${aliceBefore.tokens._map.get(
			tokenId.toString()
		)} units of token ID ${tokenId}`
	);
	var bobBefore = await new AccountBalanceQuery()
		.setAccountId(bobId)
		.execute(client);
	console.log(
		`- Bob's balance: ${bobBefore.tokens._map.get(
			tokenId.toString()
		)} units of token ID ${tokenId}`
	);
	...
	var aliceAfter = await new AccountBalanceQuery()
		.setAccountId(aliceId)
		.execute(client);
	console.log(
		`- Alice balance: ${aliceAfter.tokens._map.get(
			tokenId.toString()
		)} units of token ID ${tokenId}`
	);
	var bobAfter = await new AccountBalanceQuery()
		.setAccountId(bobId)
		.execute(client);
	console.log(
		`- Bob's balance: ${bobAfter.tokens._map.get(
			tokenId.toString()
		)} units of token ID ${tokenId}`
	);
