...
async function createNonFungibleToken() {
	...
	// Part 2 - Mint token
	const CID =
		"ipfs://Qme5niYwb7b9dFimAgJwWYFUbxJ2rbM5MguV32Z4UuALmg/alice-nft-metadata.json";
	...
	console.log(
		`- NFT association with Bob's account: ${associateAliceRx.status}\n`
	);
	...
	var balanceCheckTx = await new AccountBalanceQuery()
		...
		.execute(client);
	console.log(
		`- Alice balance: ${balanceCheckTx.tokens._map.get(
			tokenId.toString()
		)} NFTs of ID ${tokenId}`
	);
	var balanceCheckTx = await new AccountBalanceQuery()
		...
		.execute(client);
	console.log(
		`- Bob's balance: ${balanceCheckTx.tokens._map.get(
			tokenId.toString()
		)} NFTs of ID ${tokenId}`
	);
